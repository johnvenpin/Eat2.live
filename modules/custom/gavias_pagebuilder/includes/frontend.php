...
function gavias_pagebuilder_frontend( $params ) {
	...
	if ( ! empty( $content ) ) {
		foreach ( $content as $k => $row ) {
			$output .= (gavias_pagebuilder_render_el( $row ) );
		}
	}
	//$output = ob_get_clean();		
	return $output;
 	...
 	function gavias_pagebuilder_render_el( $data = array(), $content = '' ) {
				...
				if(! empty( $column['elements'])){
					foreach ($column['elements'] as $key => $element) {
						...
						if(!empty($element['row'])){
							$content_row_1 = gavias_pagebuilder_render_row($element['row']);
						}
	...
	function gavias_pagebuilder_render_row($row = array()){
			...
			if ( ! empty( $row['columns']) ){
				foreach ($row['columns'] as $key => $column) {
					...
					if(! empty( $column['elements'])){
						foreach ($column['elements'] as $key => $element) {
							if($element['element_name'] == 'gva_row'){
								$content_elements .= gavias_pagebuilder_render_row($element['row']);
							}else{
								$content_elements .= gavias_pagebuilder_render_element($element['element_name'], $element['settings']);
							}
						}
